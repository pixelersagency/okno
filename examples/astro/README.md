...
public/okno-bridge.js      Copy it from Okno → Démarrer, or from bridge/dist/
```

Set `PUBLIC_WORDPRESS_URL=https://wp.example.com` in `.env`.

**Publishing mode.** This site is static: content changes need a rebuild. In Okno's settings choose **Build hook** (Netlify, Cloudflare Pages, Vercel) or **GitHub Actions**. With Astro's server output (`output: 'server'`), choose **Contenu en direct** instead.
